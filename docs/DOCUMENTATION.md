...
ライブラリ利用者が直接触れるものは **英語** で記述する。
利用者がコードを書く過程で目にする情報を、言語の壁なく届けるための方針。

| 対象                             | 言語     | 理由                                     |
| -------------------------------- | -------- | ---------------------------------------- |
| パッケージ README                | **英語** | ライブラリ利用者が最初に読むドキュメント |
| JSDoc（`@param`, `@returns` 等） | **英語** | エディタ上で利用者に直接表示される       |
| 型名・関数名・エラーメッセージ   | **英語** | コード上のインターフェース               |
| ルート README.md                 | **英語** | プロジェクトの入口                       |

開発チーム向けのドキュメントは **日本語** で記述する。

| 対象                           | 言語       | 理由                                   |
| ------------------------------ | ---------- | -------------------------------------- |
| ルート README.ja.md            | **日本語** | 日本語話者向けのプロジェクト概要       |
| Showcase README                | **日本語** | 開発チーム内の知見共有                 |
| `docs/` 配下                   | **日本語** | コントリビューター向けの設計・方針文書 |
| コード内コメント（JSDoc 以外） | **日本語** | 開発チームの内部メモ                   |
| CLAUDE.md                      | **日本語** | 開発ガイド                             |
...
JSDoc のテキストは **英語** で記述する（「利用者インターフェースは英語」原則に基づく）。
エディタのホバーや型定義ジャンプで表示される JSDoc は、パッケージ README と同じく利用者向けインターフェースである。
...
開発セッション中に発見したパターン、設計判断、躓きを、プロジェクトの知識資産として定着させる。

### 配置の判断基準

| 問い                                                        | 配置先                      |
| ----------------------------------------------------------- | --------------------------- |
| **「どう書くか」のルール** — コーディングパターン、命名規則 | `docs/CONVENTIONS.md`       |
| **「なぜこうなったか」** — 設計判断の経緯、教訓             | `docs/PROJECT_KNOWLEDGE.md` |
| **「どう使うか」の具体例で躓いた** — 実装上の注意点         | `examples/*/README.md`      |
| **「何ができるか」が変わった** — API の追加・変更           | `packages/*/README.md`      |
| **繰り返し行うワークフロー** — 手順化したい作業             | `.claude/skills/`           |
| **まだ分類できない、または一時的なメモ**                    | MEMORY.md                   |

迷ったときの目安:

- **他の人にも役立つ** → `docs/`
- **繰り返し使うワークフロー** → `.claude/skills/`
- **Claude の行動指針として残したい** → MEMORY.md
...
MEMORY.md（メモ）
    ↓ 振り返り・整理
docs/（ドキュメント）または .claude/skills/（skill）
```
...
- 作業の大きな区切り
- MEMORY.md が長くなってきたとき
- セッション振り返り（`/session-retrospective`）の実行時

### セッション振り返り

作業の区切りで `.claude/skills/session-retrospective` を使い、知見の抽出と落とし込みを行う。
詳細な手順は skill 内に定義されている。
