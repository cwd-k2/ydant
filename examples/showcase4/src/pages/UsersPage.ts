...
export const UsersPage: Component = () =>
  div({ class: "p-6" }, function* () {
    yield* h1({ class: "text-2xl font-bold mb-4" }, "Users");
    ...
    // ユーザーリスト（リアクティブ）
    yield* reactive(() => [
      ul({ class: "space-y-2" }, function* () {
        for (const user of users()) {
          yield* li(
            {
              class: "p-3 bg-gray-50 dark:bg-slate-800 rounded flex justify-between items-center",
            },
            function* () {
