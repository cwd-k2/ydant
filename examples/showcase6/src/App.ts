        ...
        yield* ul(function* () {
          ...
          for (const post of posts) {
            yield* li(() => [
              ...
              h3(() => [classes("font-medium", "text-gray-100"), text(post.title)]),
              p(() => [
        ...
        yield* div(function* () {
          ...
          for (const user of users) {
            yield* div(() => [
              ...
              h3(() => [classes("font-medium", "text-blue-300"), text(user.name)]),
              p(() => [classes("text-sm", "text-blue-400"), text(user.email)]),
              p(() => [
                classes("text-xs", "text-blue-400", "mt-1"),
...
function ErrorDemoSection(): Render {
  ...
  const renderContent = function* () {
    ...
    if (!showError) {
      yield* div(() => [
        classes("text-center"),
        p(() => [classes("text-gray-400", "mb-4"), text("Click the button to simulate an error.")]),
        button(function* () {
    ...
    if (isLoading) {
      ...
      yield* ul(function* () {
        ...
        for (const post of data!) {
          yield* li(() => [
            classes("p-2", "bg-slate-800", "rounded", "border", "border-slate-700"),
            text(post.title),
          ]);
        }
      });
      ...
      yield* div(() => [
        classes("text-center"),
        p(() => [classes("text-gray-400", "mb-4"), text("No data loaded yet.")]),
        button(function* () {
          yield* classes(
            "px-4",
            "py-2",
